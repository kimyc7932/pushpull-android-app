...
public class DeviceOnlineMediaPlayActivityNew extends AppCompatActivity implements View.OnClickListener, IGetDeviceInfoCallBack.IDeviceCacheCallBack {
    ...
    private LinearLayout llQuickController, llSiren, llLight, llVideoContent, llVideo, llSpeak, llScreenShot, llCloudStage, llFullScreen, llSound, llPlayStyle, llPlayPause, llDetail, llBack, llVideo1, llSpeak1, llScreenShot1, llCloudStage1;
    private ImageView ivSiren, ivLight, ivPalyPause, ivPlayStyle, ivSound, ivCloudStage, ivScreenShot, ivSpeak, ivVideo, ivCloudStage1, ivScreenShot1, ivSpeak1, ivVideo1;
    ...
    private void timeZoneConfigByDay() {
        final Context ctx = getApplicationContext();
        final String deviceId = deviceListBean.deviceId;
        Log.d(TAG, "calibrationTime - start!!! ");
        String timeZoneStr = PreferencesHelper.getInstance(getApplicationContext()).getString(deviceId+"_"+ACONST.TIME_ZONE, "");
        if(timeZoneStr != null && !timeZoneStr.equals("")) return;

        // default timezone - seoul, utc+09:00
        final TimeZoneInfo timeZoneInfo = new TimeZoneInfo("84", "14", "Seoul", "UTC+09:00");
        DeviceTimezoneService.getInstance().setTimezone(deviceId, timeZoneInfo, new IDeviceOperationCallback() {
            @Override
            public void onSuccess(String response) {
                Log.d(TAG, "timeZoneConfigByDay.onSuccess -----------------------------------");
                Log.d(TAG, "timeZoneConfigByDay : " + response);
                PreferencesHelper.getInstance(ctx).set(deviceId+"_"+ACONST.TIME_ZONE, timeZoneInfo.getUtc()+" "+timeZoneInfo.getCity());
                Log.d(TAG, "timeZoneConfigByDay.onSuccess -----------------------------------");
            }

            @Override
            public void onError(Throwable throwable) {
                Log.d(TAG, "timeZoneConfigByDay.onError -----------------------------------");
                Log.d(TAG, "timeZoneConfigByDay : "+ throwable.getMessage());
                Log.d(TAG, "timeZoneConfigByDay.onError -----------------------------------");
            }
        });
        Log.d(TAG, "timeZoneConfigByDay - fin!!! ");
    }
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        ...
        if(!isPip) {
            ...
            frRecord.setVisibility(View.VISIBLE);
            llQuickController.setVisibility(View.VISIBLE);
            ...
            if (isPip) {
                ...
                frRecord.setVisibility(View.GONE);
                llQuickController.setVisibility(View.GONE);
            }
        }
    }
...
//            startActivity(intent);
    ...
    private void getLocalRecord() {
        ...
        localRecordsData.data.beginTime = ACONST.calculateHourTime(DateHelper.getTodayTimeHMS(System.currentTimeMillis()), -3);
        ...
        localRecordsData.data.queryRange = "1-100";
        localRecordsData.data.count = "100";
        deviceRecordService.queryLocalRecords(localRecordsData, new IGetDeviceInfoCallBack.IDeviceLocalRecordCallBack() {
            ...
            public void deviceLocalRecord(LocalRecordsData.Response result) {
                ...
                if (localRecords != null && localRecords.size() > 0) {
                    ...
                    for (int i=localRecords.size()-1; i>0; i--) {
                        LocalRecordsData.ResponseData.RecordsBean recordsBean = localRecords.get(i);
                        RecordsData recordsData = RecordsData.parseLocalData(recordsBean, localRecordsData.data.deviceId);
    ...
    private void initView() {
        llQuickController = findViewById(R.id.ll_quick_controller);
        ...
        llSiren = findViewById(R.id.ll_siren);
        ivSiren = findViewById(R.id.iv_siren);
...
//        deviceListBean = (DeviceDetailListData.ResponseData.DeviceListBean) bundle.getSerializable(MethodConst.ParamConst.deviceDetail);
        ...
        /** 공유된 기기인지 확인 **/
        if(deviceListBean.isShared()) {
            llDetail.setVisibility(View.GONE);
        }

        /** siren 지원여부 확인 **/
        if(!deviceListBean.deviceAbility.toLowerCase().contains(CAPABILITY.SIREN.toLowerCase())) {
            llSiren.setVisibility(View.GONE);
        } else {
            llSiren.setOnClickListener(this);
        }

        /** light 지원여부 확인 **/
        if(!deviceListBean.deviceAbility.toLowerCase().contains(CAPABILITY.WHITE_LIGHT.toLowerCase())) {
    ...
    private void setWindowListener(LCOpenSDK_PlayRealWindow playWin) {
        ...
        playWin.setPlayRealListener(new LCOpenSDK_PlayRealListener() {
            ...
            public void onIVSInfo(int winID, int ivsDirection) {
                ...
                mHandler.sendEmptyMessageDelayed(1000, 2000);//两秒消失
            }
...
//        stopRecord();
    ...
    public void setDeviceStatus(CameraStatusVO cameraStatusVO, IDeviceCameraStatus.CommonCallback callback) {
        DeviceCameraStatusService.getInstance().setDeviceCameraStatus(cameraStatusVO, callback);
    }

    boolean sirenValue = false;
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.ll_siren) {
            ...
            if(sirenValue) {
                onOff = "on";
                ivSiren.setImageResource(R.mipmap.lc_ksw_siren_off);
            }
            else {
                onOff = "off";
                ivSiren.setImageResource(R.mipmap.lc_ksw_siren_on);
            }

            CameraStatusVO cameraStatusVO = DeviceAbilityHelper.createCameraStatusVO(deviceListBean.deviceId, "0");
            cameraStatusVO.setEnableType(CAPABILITY.SIREN);
            cameraStatusVO.setEnableValue(sirenValue);

            final String finalOnOff = onOff;
            setDeviceStatus(cameraStatusVO, new IDeviceCameraStatus.CommonCallback() {
                @Override
                public void onSuccess(String response) {
                    GlobalFunc.showToast(getApplicationContext(), getText(R.string.lc_text_siren)+" "+ finalOnOff);
                }

                @Override
                public void onError(Throwable throwable) {
                    sirenValue = !sirenValue;
                    if(sirenValue) ivSiren.setImageResource(R.mipmap.lc_ksw_siren_off);
                    else ivSiren.setImageResource(R.mipmap.lc_ksw_siren_on);
                }
            });
            ...
            if (lightValue) {
                onOff = "on";
                ivLight.setImageResource(R.mipmap.lc_ksw_light_off);
            }
            else {
                onOff = "off";
                ivLight.setImageResource(R.mipmap.lc_ksw_light_on);
            }

            CameraStatusVO cameraStatusVO = DeviceAbilityHelper.createCameraStatusVO(deviceListBean.deviceId, "0");
            cameraStatusVO.setEnableType(CAPABILITY.WHITE_LIGHT);
            cameraStatusVO.setEnableValue(lightValue);

            final String finalOnOff = onOff;
            setDeviceStatus(cameraStatusVO, new IDeviceCameraStatus.CommonCallback() {
                @Override
                public void onSuccess(String response) {
                    GlobalFunc.showToast(getApplicationContext(), getText(R.string.lc_text_light)+" "+ finalOnOff);
                }

                @Override
                public void onError(Throwable throwable) {
                    lightValue = !lightValue;
                    if (lightValue) ivLight.setImageResource(R.mipmap.lc_ksw_light_off);
                    else ivLight.setImageResource(R.mipmap.lc_ksw_light_on);
                }
            });
