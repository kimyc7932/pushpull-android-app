...
public class BindSuccessPresenter implements BindSuccessConstract.Presenter {
    private final static String TAG = BindSuccessPresenter.class.getSimpleName();
    ...
    public void modifyDevName() {
        ...
        final String sc = DeviceAddModel.newInstance().getDeviceInfoCache().getDevicePwd();
        final String model = DeviceAddModel.newInstance().getDeviceInfoCache().getDeviceModel();
        final String nc = model.equalsIgnoreCase("bangtantv 1000") ? "008" : "018";
        final String devjson = "{SN:"+sn+",DT:"+model+",SC:"+sc+",NC:"+nc+"}";
            ...
            public void handleBusiness(Message msg) {
                if (mView.get() != null && mView.get().isViewActive()) {
                    ...
                    if (msg.what == HandleMessageCode.HMC_SUCCESS) {
                        refreshDevice(true);
                        // lesc0 for device add sn, model, devName
                        Log.d(TAG, "modifyDevName - "+sn);
                        Log.d(TAG, "modifyDevName - "+model);
                        Log.d(TAG, "modifyDevName - "+devName);
                        Log.d(TAG, "devjson - "+devjson);
                        ...
                        bundle.putString("devjson", devjson);
                        EventBus.getDefault().post(new DeviceAddEvent(DeviceAddEvent.DEVICE_ADDED,bundle));
                    } else {
        ...
        final LCBusinessHandler handler = new LCBusinessHandler() {
            ...
            public void handleBusiness(Message msg) {
                ...
                if (msg.what == HandleMessageCode.HMC_SUCCESS) {
                    ...
                    mView.get().deviceName(response.data.deviceList.get(0).deviceName);
                } else {
                    //失败
                    mView.get().showToastInfo(BusinessErrorTip.getErrorTip(msg));
                }
        ...
        new BusinessRunnable(handler) {
            ...
            public void doBusiness() throws BusinessException {
                try {
                    ...
                    deviceListBean.productId = DeviceAddModel.newInstance().getDeviceInfoCache().getProductId();
                    deviceListBeans.add(deviceListBean);
                    deviceDetailListData.data.deviceList = deviceListBeans;
                    //获取设备详情列表
                    DeviceDetailListData.Response result = DeviceAddOpenApiManager.listDeviceDetailsByIds(deviceDetailListData);
