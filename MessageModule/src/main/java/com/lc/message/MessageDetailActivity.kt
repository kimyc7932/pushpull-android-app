...
import com.mm.android.mobilecommon.views.MessageDateView
...
import com.mm.android.mobilecommon.route.ProviderManager.getPlaybackProvider
...
class MessageDetailActivity : AppCompatActivity(),
    ...
    MessageAdapter.OnMessageItemClickListener {
    ...
    private var deviceListBean: com.mm.android.mobilecommon.entity.device.DeviceDetailListData.ResponseData.DeviceListBean? = null
    ...
    private fun initData() {
        ...
        deviceListBean = intent.getSerializableExtra("deviceListBean") as com.mm.android.mobilecommon.entity.device.DeviceDetailListData.ResponseData.DeviceListBean?
...
//        resultIV.setImageResource(R.mipmap.pic_quesheng2)
    ...
    override fun onMessageItemClick(position: Int) {
        ...
        val alarmMessage = messageList[position]
        getLocalData(alarmMessage)
    }

    fun getLocalData(alarmMessage: AlarmMassge) {

        LocalRecordsData().apply {
            data.channelId = alarmMessage.channelId
            data.deviceId = alarmMessage.deviceId
            data.beginTime = ACONST.calculateSecondTime(alarmMessage.conv_ins_dt, -30)
            data.endTime = ACONST.calculateSecondTime(alarmMessage.conv_ins_dt, 30)
            data.type = "All"
            data.queryRange = 1.toString() + "-" + (1 + 30 - 1)

            DeviceRecordService.newInstance().queryLocalRecords(this, localRecordCallback)
        }
    }

    val localRecordCallback = object : IGetDeviceInfoCallBack.IDeviceLocalRecordCallBack {
        override fun deviceLocalRecord(result: com.lc.media.entity.LocalRecordsData.Response?) {
            if (result == null || result.data == null || result.data.records == null) {
                Toast.makeText(applicationContext, "No data", Toast.LENGTH_SHORT).show()
                DialogUtils.dismiss()
                return
            }
            val recordsBean = result.data.records[0]
            val recordData = RecordsData()
            recordData.beginTime = recordsBean.beginTime
            recordData.channelId = recordsBean.channelID
            recordData.channelID = recordsBean.channelID
            recordData.deviceId = deviceId
            recordData.endTime = recordsBean.endTime
            recordData.fileLength = recordsBean.fileLength
            recordData.size = recordsBean.fileLength.toString()
            recordData.recordId = recordsBean.recordId
            recordData.type = recordsBean.type

//            val bundle = Bundle()
//            bundle.putSerializable(
//                MethodConst.ParamConst.deviceDetail,
//                deviceListBean
//            )
//            bundle.putSerializable(MethodConst.ParamConst.recordData, recordData)
//            bundle.putInt(
//                MethodConst.ParamConst.recordType,
...
//            )
            ...
            getPlaybackProvider()!!.gotoPlayback(
                gson.toJson(deviceListBean),
