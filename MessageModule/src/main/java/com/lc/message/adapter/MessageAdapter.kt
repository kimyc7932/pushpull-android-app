...
class MessageAdapter(
    ...
    private lateinit var onMessageItemClickListener: MessageAdapter.OnMessageItemClickListener
    ...
    override fun onBindViewHolder(holder: MessageHolder, position: Int) {
        ...
        if (alarmMassge.msgType.contains("abAlarmSound")) {
            messageType = mContext.getText(R.string.lc_sound_message_text) as String
        } else {
            messageType = mContext.getText(R.string.lc_motion_message_text) as String
        }
        alarmMassge.conv_ins_dt = alarmMassge.localDate
        holder.msgContentTv.text =  "[ "+TimeUtils.string2String(alarmMassge.localDate,TimeUtils.LONG_FORMAT,TimeUtils.SHORT_FORMAT)+" ] "+if (TextUtils.isEmpty(alarmMassge.msgType)) mContext.getText(R.string.common_other) else messageType
//        holder.msgTimeTv.text = TimeUtils.string2String(alarmMassge.localDate,TimeUtils.LONG_FORMAT,TimeUtils.SHORT_FORMAT)
        ...
        holder.msgContentTv.setOnClickListener {
    ...
    fun setOnMessageItemClickListener(onMessageItemClickListener: MessageDetailActivity) {
